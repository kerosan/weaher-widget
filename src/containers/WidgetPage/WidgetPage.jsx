...
import {RaisedButton, SvgIcon, Tab, Tabs} from "material-ui";
            ...
            <Tabs>
                {
                    locations.places.map((tab, key) => {
                        ...
                        return <Tab
                            key={key}
                            icon={<CloseIcon onClick={() => this.closeTab(tab)}/>}
                            className={'Tab'}
                            label={tab.formatted_address}>

                            {`Temperature: ${tab.weather.temp}`}
                            <br/>
                            {`Pressure: ${tab.weather.pressure}`}
                        </Tab>
                    })
